...
from .utils import resolve_action_type, resolve_generated_type, get_or_create_topic
...
class ActionServer:
    ...
    def __init__(
        ...
        *,
        ...
        self._results: dict[bytes, tuple[int, object | None]] = {}
        ...
        self._send_goal_topic = f"{action_name.strip('/')}/_action/send_goal"
        self._get_result_topic = f"{action_name.strip('/')}/_action/get_result"
        self._feedback_topic = f"{action_name.strip('/')}/_action/feedback"
        self._cancel_topic = f"{action_name.strip('/')}/_action/cancel_goal"
    ...
    def _on_get_result(self, request_msg):
        ...
        goal_id = getattr(request_msg, "goal_id", None)
        key = _uuid_bytes(goal_id)
        status, result_obj = self._results.get(key, (0, None))
        try:
            ...
            if hasattr(res_msg, "goal_id"):
                setattr(res_msg, "goal_id", _copy_goal_id(goal_id))
        except Exception:
            pass
            ...
            if result is not None:
                for attr in dir(result):
                    ...
                    try:
                        ...
                        pass
        with self._lock:
            self._results[key] = (status_code, result_obj)
